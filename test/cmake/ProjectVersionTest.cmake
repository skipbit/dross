...
# test/CMakeLists.txt.
#
# dross_version_detect is covered against real repositories built here. It is
# where the tag selection, the working directory and the fallbacks live, and a
# defect in it reaches SOVERSION, dross.pc and drossConfigVersion.cmake with
...
if(NOT SCRATCH_DIR)
    ...
    set(SCRATCH_DIR "${SCRATCH_DIR}/dross-version-test")
endif()
...
macro(expect_version TAG EXPECTED)
    dross_version_from_tag(actual "${TAG}")
...
if(NOT GIT_FOR_TEST)
    ...
    math(EXPR MINIMUM_CHECKS "${MINIMUM_CHECKS} + 58")
    ...
    macro(expect_detect DIR EXPECTED_VERSION DESCRIBE_PATTERN LABEL)
        dross_version_detect(got_version got_describe "${DIR}")
        math(EXPR checks "${checks} + 1")
    ...
    dross_version_detect(checkout_version checkout_describe "${SCRATCH_DIR}/archived")
    ...
    dross_version_detect(promoted_version promoted_describe "${SCRATCH_DIR}/promoted")
    ...
    # Sources vendored inside another project: git finds that project's
    # repository, whose tags say nothing about this one.
    make_repo("${SCRATCH_DIR}/host")
    create_tag("${SCRATCH_DIR}/host" "v9.9.9")
    file(MAKE_DIRECTORY "${SCRATCH_DIR}/host/vendor/dross")
    expect_detect("${SCRATCH_DIR}/host/vendor/dross" "0.0.0" "^unknown$" "sources inside another repository")
