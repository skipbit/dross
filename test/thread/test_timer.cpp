...
void spin_until(dross::runloop& loop, Predicate&& predicate,
                std::chrono::milliseconds bound = kTimeout)
{
...
TEST(timer_test, a_one_shot_fires_once_and_then_invalidates_itself)
{
    ...
    dross::timer t = dross::timer::once(
        std::chrono::milliseconds{0}, [&fire_count](dross::timer) { ++fire_count; }, loop);

    spin_until(loop, [&fire_count]() { return fire_count > 0; });
...
TEST(timer_test, a_repeating_timer_fires_more_than_once_and_stays_valid)
{
    ...
    dross::timer t = dross::timer::repeating(
        std::chrono::milliseconds{1}, [&fire_count](dross::timer) { fire_count.fetch_add(1); },
        loop);

    spin_until(loop, [&fire_count]() { return fire_count.load() >= 3; });
...
TEST(timer_test, the_callback_receives_the_timer_and_can_invalidate_itself_from_inside)
{
    ...
    spin_until(loop, [&t]() { return !t.valid(); });
...
TEST(timer_test, invalidate_is_idempotent_and_safe_after_the_timer_has_ended)
{
    ...
    dross::timer t2 = dross::timer::once(std::chrono::milliseconds{50}, [](dross::timer) {}, loop);
    t2.invalidate();
    t2.invalidate();
...
TEST(timer_test, a_copied_handle_names_the_same_timer)
{
    ...
    dross::timer t = dross::timer::once(std::chrono::milliseconds{1000}, [](dross::timer) {}, loop);
    dross::timer copy = t;
...
TEST(timer_test, installing_on_an_explicit_loop_runs_on_that_loops_thread)
{
    ...
    dross::timer t = dross::timer::once(
        std::chrono::milliseconds{0},
        [&ran_on](dross::timer) { ran_on.store(dross::current_thread().native_id().value()); },
        *worker_loop);
    static_cast<void>(t);
...
TEST(timer_test, timer_count_rises_on_install_and_falls_on_invalidate_and_after_a_one_shot_fires)
{
    ...
    dross::timer repeating =
        dross::timer::repeating(std::chrono::milliseconds{1000}, [](dross::timer) {}, loop);
    EXPECT_EQ(loop.timer_count(), 1U);

    dross::timer one_shot = dross::timer::once(std::chrono::milliseconds{0}, [](dross::timer) {}, loop);
    EXPECT_EQ(loop.timer_count(), 2U);

    spin_until(loop, [&one_shot]() { return !one_shot.valid(); });
    EXPECT_EQ(loop.timer_count(), 1U);
...
TEST(timer_test, run_pending_fires_a_timer_already_due_and_not_one_that_is_not)
{
    ...
    int due_fired = 0;
    int later_fired = 0;
    dross::timer due = dross::timer::once(
        std::chrono::milliseconds{0}, [&due_fired](dross::timer) { ++due_fired; }, loop);
    dross::timer later = dross::timer::once(
        std::chrono::milliseconds{1000}, [&later_fired](dross::timer) { ++later_fired; }, loop);
...
TEST(timer_test, a_late_fire_is_not_made_up)
{
    ...
    dross::timer t = dross::timer::repeating(
        ...
        [&fire_count](dross::timer) {
            fire_count.fetch_add(1);
            // Blocks well past several intervals, so a naive implementation
            // that queued up every missed deadline would fire many times in
            // a row once this returns.
            std::this_thread::sleep_for(std::chrono::milliseconds{100});
        },
        loop);

    loop.run_for(std::chrono::milliseconds{150});

    EXPECT_GE(fire_count.load(), 1);
    EXPECT_LE(fire_count.load(), 2);
...
TEST(timer_test, invalidate_wakes_a_loop_blocked_in_its_wait)
{
    dross::thread worker;
    std::optional<dross::runloop> worker_loop;
    std::atomic<bool> captured{false};
    ...
    dross::timer far =
        dross::timer::once(std::chrono::seconds{10}, [](dross::timer) {}, *worker_loop);
    std::this_thread::sleep_for(std::chrono::milliseconds{50});

    std::atomic<bool> soon_fired{false};
    far.invalidate();
    dross::timer soon = dross::timer::once(
        std::chrono::milliseconds{100}, [&soon_fired](dross::timer) { soon_fired.store(true); },
        *worker_loop);

    // Bounded well under far's original 10-second deadline: if invalidating
    // far had not woken the parked loop, this would only settle once that
    // stale deadline finally timed out on its own.
...
TEST(timer_test, invalidate_from_another_thread_while_the_loop_is_running_it)
{
    ...
    ASSERT_TRUE(worker.perform([&t, &installed, &fire_count]() {
        t = dross::timer::repeating(std::chrono::milliseconds{1},
                                    [&fire_count](dross::timer) { fire_count.fetch_add(1); });
