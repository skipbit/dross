...
TEST(toml_test, empty_document) {
    ...
    EXPECT_TRUE(result.value().as<dictionary>().empty());
}
...
TEST(toml_test, simple_key_value) {
    ...
    EXPECT_EQ(std::string(dict["key"]), "value");
}
...
description = "A test configuration"
    ...
    EXPECT_EQ(std::string(dict["title"]), "Example");
    EXPECT_EQ(std::string(dict["version"]), "1.0.0");
    EXPECT_EQ(std::string(dict["description"]), "A test configuration");
}
...
TEST(toml_test, string_with_escapes) {
    ...
    auto dict = result.value().as<dictionary>();
    EXPECT_EQ(std::string(dict["text"]), "Line 1\nLine 2\tTabbed");
}
...
TEST(toml_test, quoted_keys) {
    ...
    EXPECT_EQ(std::string(dict["special key"]), "value");
}
...
empty = []
    ...
    EXPECT_EQ(std::string(strings[0]), "alpha");
    EXPECT_EQ(std::string(strings[2]), "gamma");
    
    // Test mixed array
    auto mixed = dict["mixed"].as<array>();
    EXPECT_EQ(mixed.size(), 3u);
    EXPECT_EQ(std::string(mixed[0]), "text");
...
connection_max = 5000
    ...
    auto dict = result.value().as<dictionary>();
    EXPECT_EQ(std::string(dict["title"]), "Main");
    
    auto database = dict["database"].as<dictionary>();
    EXPECT_EQ(std::string(database["server"]), "192.168.1.1");
...
dc = "eqdc10"
    ...
    EXPECT_EQ(std::string(alpha["ip"]), "10.0.0.1");
    EXPECT_EQ(std::string(alpha["dc"]), "eqdc10");
    
    auto beta = servers["beta"].as<dictionary>();
    EXPECT_EQ(std::string(beta["ip"]), "10.0.0.2");
    EXPECT_EQ(std::string(beta["dc"]), "eqdc10");
}
...
TEST(toml_test, serialize_simple) {
    dictionary dict;
    dict["title"] = string{"Example"};
    dict["version"] = number{"1.0"};
    dict["enabled"] = boolean{true};
...
TEST(toml_test, serialize_array) {
    dictionary dict;
    array numbers;
    numbers.push_back(value{number{"1"}});
    numbers.push_back(value{number{"2"}});
    numbers.push_back(value{number{"3"}});
...
items = [1, 2, 3])";
    ...
    auto dict1 = parsed.value().as<dictionary>();
    auto dict2 = reparsed.value().as<dictionary>();
    
    EXPECT_EQ(std::string(dict1["title"]), std::string(dict2["title"]));
...
ports = [8080, 8443]
    ...
    auto original_dict = parsed.value().as<dictionary>();
    auto roundtrip_dict = reparsed.value().as<dictionary>();
    
    EXPECT_EQ(std::string(original_dict["title"]), std::string(roundtrip_dict["title"]));
    
    auto orig_db = original_dict["database"].as<dictionary>();
    auto rt_db = roundtrip_dict["database"].as<dictionary>();
    EXPECT_EQ(std::string(orig_db["host"]), std::string(rt_db["host"]));
...
TEST(toml_test, whitespace_handling) {
   ...
   key1   =   "value1"   
    ...
    auto dict = result.value().as<dictionary>();
    EXPECT_EQ(std::string(dict["key1"]), "value1");
...
empty_array = []
    ...
    auto dict = result.value().as<dictionary>();
    EXPECT_EQ(std::string(dict["empty_string"]), "");
...
TEST(toml_test, unicode_strings) {
    ...
    auto dict = result.value().as<dictionary>();
    EXPECT_EQ(std::string(dict["unicode"]), "Hello, 世界! 🌍");
}

// Serialization Error Tests
TEST(toml_test, serialize_non_dictionary_root) {
