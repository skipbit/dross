...
function(dross_version_release_tags OUT_TAGS TEXT SEPARATOR PREFIX)
    ...
    foreach(entry IN LISTS entries)
        ...
        dross_version_from_tag(candidate "${entry}")
...
# ARGN is.
function(dross_version_preferred_tag OUT_TAG)
    set(best "")
    set(best_version "")
...
function(dross_describe_commit SOURCE_DIR OUT_DESCRIPTION)
    ...
    execute_process(
        COMMAND ${DROSS_GIT_EXECUTABLE} status --porcelain --untracked-files=no
        WORKING_DIRECTORY ${SOURCE_DIR}
...
function(dross_version_from_repository SOURCE_DIR OUT_FOUND OUT_VERSION OUT_DESCRIBE)
    ...
    if(list_result EQUAL 0)
        dross_version_release_tags(release_tags "${all_tags}" "\n" "")
    else()
        set(release_tags "")
    endif()
    ...
    # happens to name it.
    execute_process(
        COMMAND ${DROSS_GIT_EXECUTABLE} describe --tags --abbrev=0 ${selectors}
        WORKING_DIRECTORY ${SOURCE_DIR}
    ...
    execute_process(
        COMMAND ${DROSS_GIT_EXECUTABLE} tag --points-at "${nearest}^{commit}"
        WORKING_DIRECTORY ${SOURCE_DIR}
        OUTPUT_VARIABLE here
        OUTPUT_STRIP_TRAILING_WHITESPACE
        ERROR_QUIET
    )
    dross_version_release_tags(here_tags "${here}" "\n" "")
    dross_version_preferred_tag(chosen ${here_tags})
    if(NOT chosen)
        set(chosen "${nearest}")
    endif()
    dross_version_from_tag(version "${chosen}")
    ...
    # same distance away.
    execute_process(
        COMMAND ${DROSS_GIT_EXECUTABLE} describe --tags --always --dirty "--match=${nearest}"
        WORKING_DIRECTORY ${SOURCE_DIR}
...
function(dross_version_from_archive SOURCE_DIR OUT_FOUND OUT_VERSION OUT_DESCRIBE)
    ...
    dross_version_release_tags(tags "${refs}" ", " "tag: ")
    dross_version_preferred_tag(chosen ${tags})
...
function(dross_version_detect OUT_VERSION OUT_DESCRIBE SOURCE_DIR)
    dross_version_from_repository("${SOURCE_DIR}" found detected_version detected_describe)
    if(NOT found)
        dross_version_from_archive("${SOURCE_DIR}" found detected_version detected_describe)
    endif()
    if(NOT found)
